...
#    'fixup_resnet50': 'https://download.pytorch.org/models/resnet50-19c8e357.pth',
        ...
        for m in self.modules():
            if isinstance(m, FixupBasicBlock):
                ...
                nn.init.normal_(
                    ...
                    mean=0,
                    std=np.sqrt(
                        2 / (m.conv1.weight.shape[0] * np.prod(m.conv1.weight.shape[2:]))
...
def fixup_resnet18(pretrained=False, progress=True, **kwargs):
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('fixup_resnet18', FixupBasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
